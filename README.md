...
- Pages 주소가 생성된 뒤 휴대폰에서도 접속해 PDF 다운로드와 코드 복사 버튼이 동작하는지 확인하세요.
- 3회차 코드 예제 3개는 `index.html`의 복사 버튼과 `script.js` 예제 본문으로 연결합니다.
- 문서를 교체할 때는 기존 파일명 `docs/lesson-01-review.pdf`, `docs/lesson-02-training.pdf`, `docs/lesson-03-script.pdf`를 그대로 유지하면 링크를 다시 고칠 필요가 없습니다.
